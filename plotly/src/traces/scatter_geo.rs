    ...
    Lon: Serialize + Clone,
{
    ...
    /// Sets a reference between this trace's data coordinates and a geo
    /// subplot. If "geo" (the default value), the data refer to
    /// `layout.geo`. If "geo2", the data refer to `layout.geo2`, and
    /// so on.
    ...
    /// Determines if this scattergeo trace's layers are to be inserted
    /// before the layer with the specified ID. By default, scattergeo
    /// layers are inserted above all the base layers. To place the
    /// scattergeo layers above every other layer, set `below` to "''".
    below: Option<String>,
...
mod tests {
    ...
    #[test]
    fn serialize_scatter_geo() {
        let scatter_geo = ScatterGeo::new(vec![45.5017], vec![-73.5673])
            .name("name")
